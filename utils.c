...
void generatecode(station *estacao, int total)
{
    int i, flag;
    time_t t;
    char codechars[36] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; // Array de caracteres que podem ser usados para gerar o c�digo.
    char temp;
    ...
    do
    {
        ...
        // Verifica se o c�digo gerado j� est� acossiado a outra esta��o.
        for(i = 0; i<total; i++)
        {
...
// Fun��o que verifica se uma esta��o est� numa linha.
int station_is_in_line(pline p,char *codigo)
{
    int i;
...
// Fun��o que verifica se duas linhas t�m esta��es em comum.
...
void guarda_dados(pstation paragens, int total, pline linhas)
{
    ...
    // Escreve o total de esta��es, e depois escreve as esta��es.
    fwrite(&total,sizeof(int),1,f);
    fwrite(paragens,sizeof(station),total,f);
    fclose(f);

    f = fopen("linhas.dat","w+b");
    // Escreve cada linha e o array dinamico de esta��es que cada linha tem.
    while(linhas != NULL)
    {
...
pstation carrega_estacoes(int *total)
{
    ...
    // Aloca memoria para "total" de esta��es.
    ...
    // Le as esta��es um total de vezes.
...
pline carrega_linhas(pline p)
{
    ...
    while(fread(&linha, sizeof(line), 1, f) == 1){
        ...
        *novo = linha;
        // Aloca memoria para o array de esta��es da linha.
        novo->stations_in_line = malloc(sizeof(station) * novo->station_count);
        // Le o array de esta��es da linha.
